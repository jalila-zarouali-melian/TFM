...
def detectar_outliers(dataset, umbral=3):
    dataset_outliers = dataset.copy()

    for columna in dataset.columns:
        if dataset[columna].dtype in [np.float64, np.int64]:
            z_scores = np.abs((dataset[columna] - dataset[columna].mean()) / dataset[columna].std())
            dataset_outliers[columna] = np.where(z_scores > umbral, np.nan, dataset[columna])
...
def eliminar_outliers(dataset, umbral=3):
    ...
    for columna in dataset_outliers.columns:
        if dataset_outliers[columna].dtype in [np.float64, np.int64]:
...
def normalizar_dataset(dataset):
    ...
    return dataset
def one_hot_encoding(dataset, target_column):
    # Excluir temporalmente la columna objetivo del DataFrame
    ...
    dataset = pd.get_dummies(dataset, columns=categorical_columns, prefix=categorical_columns)
    dataset[target_column] = target_series
def verificar_desbalanceo(dataset, variable_objetivo):
    ...
    umbral_desbalanceo = 10  # osea, hay desbalanceo si una clase representa menos del 10% del total
    desbalanceo = porcentaje_menor_clase < umbral_desbalanceo

    return desbalanceo
def auto_resampling(X, y):
    ...
    if imbalance_ratio < 0.5:
        ...
        method_used = "RandomUnderSampler"
    else:
        sampler = SMOTEENN()
        method_used = "SMOTEENN"

    X_resampled, y_resampled = sampler.fit_resample(X, y)

    return X_resampled, y_resampled, method_used
...
if choice == "Preprocesado":
    ...
    if df is not None:
        ...
        if st.button('Ejecutar preprocesado'):
            st.subheader("Detectar outliers")
            df_con_outliers = detectar_outliers(df)
            st.dataframe(df_con_outliers)

            st.subheader("Eliminar Outliers")
            df_sin_outliers = eliminar_outliers(df)
            ...
            st.session_state.df_clean = one_hot_encoding(df_normalizado, target)
            ...
            if desbalanceo:
                ...
                X_resampled, y_resampled, resampling_method = auto_resampling(
                    st.session_state.df_clean.drop(columns=[target]), st.session_state.df_clean[target])
                st.write(f"Resampling method used: {resampling_method}")
                st.write("Resampled dataset:")
                st.dataframe(X_resampled)
                st.write("Resampled target variable:")
                st.dataframe(y_resampled)
            else:
...
if choice == "Generar nuevas predicciones":
    if 'predictions' in st.session_state:
        ...
        df_clean2 = one_hot_encoding(df_normalizado2)
